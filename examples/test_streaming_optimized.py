...
def main():
    ...
    sf.write(
        "output_streaming_baseline-ref-fixed.wav",
        result["audio"],
        result["sample_rate"],
    )
    rtf = (
    ...
    print(
        f"First chunk: {result['first_chunk_time']:.2f}s, Total: {result['total_time']:.2f}s, Chunks: {result['chunk_count']}"
    )
    print(
        f"Audio duration: {result['audio_duration']:.2f}s, Chunk duration: {result['avg_chunk_duration']*1000:.0f}ms, RTF: {rtf:.2f}"
    )
    ...
    model.enable_streaming_optimizations(
        ...
        compile_mode="max-autotune",  # You can also try "reduce-overhead" for faster compile time
    ...
    test_texts = [
        (
            "Всем привет! Это тестовый текст для озвучки! Теперь стриминг звучит хорошо сразу.",
            "short",
        ),
        (
            "Это более длинный текст для тестирования производительности стриминга. Мы хотим убедиться, что система работает стабильно на разных длинах текста.",
            "medium",
        ),
        ("Короткий тест.", "tiny"),
        (
    ...
    for i, (text, text_label) in enumerate(test_texts, 1):
        ...
        if i == 1:
            sf.write(
                "output_streaming_optimized.wav", result["audio"], result["sample_rate"]
            )
    ...
    baseline_rtf = (
        baseline_result["total_time"] / baseline_audio_dur
        if baseline_audio_dur > 0
        else 0
    )
    ...
    std = results[0]
    std_rtf = (
        std["total_time"] / std["audio_duration"]
        if std.get("audio_duration", 0) > 0
        else 0
    )
    print(
        f"{'Standard (no streaming)':<25} {'N/A':>10} {std['total_time']:>7.2f}s {std.get('audio_duration', 0):>7.2f}s {std_rtf:>6.2f} {'N/A':>7} {'N/A':>12}"
    )
    ...
    if optimized_results:
        avg_rtf = np.mean(
            [r["total_time"] / r["audio_duration"] for r in optimized_results]
        )
